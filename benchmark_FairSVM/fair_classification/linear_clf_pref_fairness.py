    ...
    def fit(self, X, y, x_sensitive, cons_params=None):
        ...
        if self.train_multiple == True:
            ...
            for k in set(x_sensitive):
                ...
                X_k = X[idx]
                y_k = y[idx]
                obj += cvx.sum_squares(w[k][1:]) * self.lam[k] # first term in w is the intercept, so no need to regularize that

                if self.loss_function == "logreg":
                    obj += cvx.sum(  logistic( cvx.multiply(-y_k, X_k*w[k]) )  ) / num_all # notice that we are dividing by the length of the whole dataset, and not just of this sensitive group. this way, the group that has more people contributes more to the loss
                    
                elif self.loss_function == "svm_linear":
                    obj += cvx.sum ( cvx.cvx.maximum (0, 1 - cvx.multiply ( y_k,  X_k*w[k])) ) / num_all
        ...
        if cons_type in [1,2,3]: # 1 - pref imp, 2 - EF, 3 - pref imp & EF
            ...
            for val in set(x_sensitive):
                ...
                for k in w.keys(): # get the distance with each group's w
                    prod_dict[val][k] = cvx.sum(  cvx.maximum(0, X_g*w[k])   ) / num_g
